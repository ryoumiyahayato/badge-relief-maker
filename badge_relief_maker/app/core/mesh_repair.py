...
from .mesh_data import as_faces, as_vertices, signed_volume, valid_face_mask


def face_count(faces):
    """Return the number of validated mesh faces."""
    return int(len(as_faces(faces)))
...
def remove_invalid_faces(vertices, faces):
    ...
    if len(faces) == 0:
        return faces, 0
    valid = valid_face_mask(len(vertices), faces)
...
def orient_closed_components_outward(vertices, faces):
    ...
    for component in components:
        ...
        if closed and signed_volume(vertices, component_faces) < 0.0:
