...
|---|---|---|
| Root package and CLI | Implemented | Local Windows Python 3.10/3.12 gates pass with 226 tests; GitHub Actions run required |
| Project v2 and migration | Implemented | Local migration/adversarial regressions pass |
| Atomic save/path containment/unique assets | Implemented | Local atomic/collision/Windows junction regressions pass |
| EXIF and alpha/luminance masks | Implemented | Generated representative fixtures pass locally |
| Perspective/manual crop/mask edits | Implemented core and GUI interaction | Automated GUI/coordinate tests pass; manual interaction record required |
| Unified coordinates | Reversible source/final-grid mapping and a preview/standard/high physical-placement fixture are implemented | Saved-project cross-quality fixture passes locally |
| Height generation and editing | Grayscale, layers, locks, set/add/subtract/smooth, global refine implemented | Current local full suite passes |
| Single-side closed solid | Implemented for current fixtures | Current local topology suite passes |
| Profiled edges | Straight, sloped, bevel and rounded builders implemented | Automated topology passes; Blender visual evidence required |
...
| Desktop GUI | Source/mask/height previews, crop, mask, layer and height tools, report, worker and unsaved handling implemented | Automated tests and packaged offscreen startup pass; manual flow pending |
| Double-side | Fused shared-body mode and separate blocked placeholder implemented | Flip/scale/rotation/offset/footprint/Z and packaged exports pass; Blender validation pending |
| Windows packaging | Versioned PyInstaller spec/script and CI artifact job implemented | Local build/CLI/GUI/sample exports pass; CI artifact and clean-machine result pending |
...
Current local working-tree evidence based on `e15ebe79` passes the engineering gate on Python 3.10.20 and 3.12.13 with `226` tests in each environment. It remains working-tree evidence until committed and exercised by GitHub Actions.
...
1. Commit the reviewed follow-up and obtain the configured GitHub Actions Python 3.10/3.12/package result; equivalent local gates already pass.
2. Run the locally passing packaged executable on a separate clean Windows machine without Python/source checkout.
